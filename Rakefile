...
namespace :layout do
  task :build do
    ...
    File.open File.join(File.dirname(__FILE__), '_includes', 'google_analytics.html'), 'w' do |f|
      f.puts ERB.new(get('https://raw.github.com/brighterplanet/brighter_planet_layout/master/app/views/layouts/_google_analytics.html.erb'), nil, nil, '@output').result(GoogleAnalytics.new.get_binding)
    end

    File.open File.join(File.dirname(__FILE__), 'stylesheets', 'brighter_planet.css'), 'w' do |f|
      f.puts get('https://raw.github.com/brighterplanet/brighter_planet_layout/master/public/stylesheets/brighter_planet.css')
    end
    
    File.open File.join(File.dirname(__FILE__), 'stylesheets', 'fonts.css'), 'w' do |f|
      f.puts get('https://raw.github.com/brighterplanet/brighter_planet_layout/master/public/stylesheets/fonts.css')
    end
    ...
    %w(bg cards logo radiant_earth-small glow).each do |image|
      File.open File.join(File.dirname(__FILE__), 'stylesheets', 'images', "#{image}.png"), 'wb' do |f|
        f.puts get("https://raw.github.com/brighterplanet/brighter_planet_layout/master/public/stylesheets/images/#{image}.png")
      end
    end
    
    File.open File.join(File.dirname(__FILE__), 'favicon.ico'), 'wb' do |f|
      f.puts get("https://raw.github.com/brighterplanet/brighter_planet_layout/master/public/favicon.ico")
    end
  end
end
